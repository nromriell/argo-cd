...
func (c *Cache) SetGitReferences(repo string, references []*plumbing.Reference) error {
	...
	return c.cache.SetItem(gitRefsKey(repo), input, &cacheutil.CacheActionOpts{Expiration: c.revisionCacheExpiration, Delete: false})
}
...
func (c *Cache) GetOrLockGitReferences(repo string, references *[]*plumbing.Reference) (updateCache bool, lockId string, err error) {
	...
	for time.Now().Before(waitUntil) {
		...
		if _, cacheReferences, err := c.GetGitReferences(repo, myLockId, cacheutil.CacheTypeInMemory); err != nil {
			return false, myLockId, err
		} else if cacheReferences != nil {
			// Successfully retrieved the key from local cache
			*references = *cacheReferences
			return false, myLockId, nil
		}
...
func (c *Cache) UnlockGitReferences(repo string, lockId string) error {
	...
	if err = c.cache.GetItem(gitRefsKey(repo), &input, nil); err == nil &&
		...
		return c.cache.SetItem(gitRefsKey(repo), input, &cacheutil.CacheActionOpts{Expiration: 0, Delete: true, CacheType: cacheutil.CacheTypeExternal})
	}
	return err
}
...
func (c *Cache) GetRevisionMetadata(repoURL, revision string) (*appv1.RevisionMetadata, error) {
	...
	return item, c.cache.GetItem(revisionMetadataKey(repoURL, revision), item, &cacheutil.CacheActionOpts{CacheType: cacheutil.CacheTypeExternal})
}
